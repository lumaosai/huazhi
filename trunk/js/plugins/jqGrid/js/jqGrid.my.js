define([cdnDomain+'/js/plugins/jqGrid/js/grid.locale-cn.js'], function (require) {
    ...
    var jqGridAll = {
         ...
         * 参数  table标签的ID属性值
         */
        headerFixed: function (tableId) {
            return   {
              scroll:true,
          }
            $(tableId).closest(".ui-jqgrid-bdiv").css({ 'overflow-y' : 'scroll' });
        },
        /**
        *功能：主从表
         * tableId :主表table标签Id
         * settings1:主表基础配置
         * data:获取从表的参数参数：{1、从表的接口   2、从表需要的参数}
         * callback：执行成功之后要做的方法
        * */
        prinsubTable:function (tableId,settings1,data,callback) {
            // #children   字表
            var confingSetting = {
                beforeSelectRow:function (rowid,e) {
                    loadChildTable(rowid)
                }
            };
            var setting = jqGridAll.setting(settings1);
            setting = $.extend(true, {},setting,confingSetting) ;
            $(tableId).jqGrid(setting,data);
            function loadChildTable(ids) {
                if(!data.contentType){
                    Api.ajaxForm("../html/0_0/json/jqGrid.json",{},function (result) {
                        if (callback){
       ...
       jqGridTable: function(tableId,settings){
         ...
         * @param sonWidth: 如果子表宽度计算需要单独调整时传参sonWidth 可以为正负数字
         */
        jqGridTableLevel: function(tableId,settings1,settings2,config,subtable,callback,sonWidth){
            var  configSettion = {
                subGrid : true,
            ...
            function bindSubGrid(subgrid_id, collectLineId) {
                ...
                if(!config.contentType){
                    Api.ajaxJson(url,dataJsons,function(result){
                        ...
                        if(sonWidth){
                            $('.subTableBox .ui-jqgrid,.subTableBox .ui-jqgrid-view,.subTableBox .ui-jqgrid-hdiv,.subTableBox .ui-jqgrid-bdiv').css('width',parseInt($('.subTableBox table').css('width').split('px')[0])+parseInt(sonWidth)+'px');
                        }else{
                            $('.subTableBox .ui-jqgrid,.subTableBox .ui-jqgrid-view,.subTableBox .ui-jqgrid-hdiv,.subTableBox .ui-jqgrid-bdiv').css('width',$('.subTableBox table').css('width'));
                        }
    ...
    return {
        ...
        jG_headerFixed:jqGridAll.headerFixed,// 固定表头
        jG_prinsubTable:jqGridAll.prinsubTable,// 主从表
