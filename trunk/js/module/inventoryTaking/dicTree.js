require([cdnDomain+'/js/zlib/app.js'], function (App) {
    var PageModule = {
        ...
        planDicInit: function () {
            ...
            require(['easyui_my'],function(easyuiObj){
                ...
                function treeGridLoad(nodeId){
                    Api.ajaxJson(Const.aps + "/api/aps/Device/ajaxTreeJson/" + nodeId, {}, function (result) {
                        if(result.success){
                            $('#tt').treegrid({
                                idField:'id',
                                treeField:'name',
                                ...
                                onBeforeExpand:function(row){
                                    ...
                                    if(!children || children.length==0){
                                        Api.ajaxJson(Const.aps+"/api/aps/Device/ajaxTreeJson/"+row.id,{},function(result){
                ...
                function btncilck(){
                    ...
                    $('.btn-delete').unbind('click').click(function () {
                        ...
                        Bus.deleteItem('确定要删除该指标吗', Const.aps + '/api/aps/Device/delete',data,deleteItemCallBack);
                    });
                    // 添加下级菜单
        ...
        planDicCheckView:function () {
            // $('#inputForm').attr('action',Const.aps+'/api/aps/Device/save');
            ...
            if (id == '' && pId == '') { //新增
                ...
                var idParam = id || pId;
                Api.ajaxJson(Const.aps+"/api/aps/Device/view/"+idParam, {}, function (result) {
                    if (result.success) {
        ...
        comForZtree: function () {
            ...
            //$('#inputForm').attr('action', Const.aps + '/api/aps/Formula/save');
            var id = Mom.getUrlParam('id');
            var paramRecord = $("#paramRecord"),
                html = "", htmlArr = [], inputArr = [];
            if (id) {
                $("#objId").val(id);
                Api.ajaxForm(Const.aps + "/api/aps/Device/view/" + id, {}, function (result) {
                    if (result.success) {
