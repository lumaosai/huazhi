            ...
            var PageModule = {
                ...
                init:function () {
                    require(['Page'], function () {
                        ...
                        //通过字典配置创建信息
                        Api.ajaxForm(Const.admin + "/api/sys/SysDict/type/STOCKTAKE_TYPE",{},function (result) {
                            if(result.success){
                                var rows = result.rows;
                                Bus.appendOptionsValue($('#oreType'), rows, 'value', 'label');
                                var btnArr = PageModule.gettype(result.rows);
                                //创建新增按钮
                               for(var i=0;i<btnArr.length;i++){
                                    var createBn = "<button type='"+btnArr[i].typeMy+"' class='tabbtn btn btn-white'>"+
                                        "<i class='iconfont icon-xinjian'></i>"+" "+"创建"+
                                        btnArr[i].label+"</button>";
                        ...
                        window.pageLoad = function () {
                            ...
                            new Page().init(Const.aps+"/api/aps/collect/page",data,true,function(result) {
                                ...
                                $(".btn-delete").unbind("click").on("click",function () {
                                    ...
                                    Bus.deleteItem('确定要删除该信息', Const.aps + '/api/aps/collect/delete', {ids:ids});
                                })
                            });
                            //多个删除
                                ...
                                if (bol) {
                                    top.layer.confirm('请您确认是否要删除勾选数据', {icon: 3, title: '系统提示'}, function (index) {
                                        if (str.length > 0) { //新增的元素+已存在的数据 或全是已存在的数据
                                            ...
                                            var url = Const.aps + '/api/aps/collect/delete';
