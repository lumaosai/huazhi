    ...
    var PageModule = {
        ...
        planTempInit: function () {
            ...
            $('#del-btn').click(function () {
                Bus.delCheckTable('您确定要删除吗', Const.aps+'/api/stocktake/StocktakeTemplate/delete','#listTable');
            });
            require(['Page'], function () {
                var page = new Page();
                var pageLoad = function () {
                    page.init(Const.aps + "/api/stocktake/StocktakeTemplate/page", {}, true, function(data) {
        ...
        planTempCheckViewInit: function(){
            ...
            if (id) {
                Api.ajaxJson(Const.aps+"/api/stocktake/StocktakeTemplate/form/"+id, {}, function(result) {
                    if (result.success) {
            ...
            function dsRender(res) {
                Api.ajaxJson(Const.admin+'/api/sys/SysDict/type/PC_TEMPLETE',{},function (result) {
                    if(result.success) {
        ...
        planTempPreserveInit: function(){
            ...
            require(['ztree_my'],function(ZTree){
                ...
                function ztreeLoad(){
                    var apiUrl = Const.aps+'/api/stocktake/StocktakeTemplate/formTemplateDetail/'+tempId;
                ...
                $('#btn-addSon').click(function () {
                    ...
                    if(selResult.success){
                        ...
                        var apiCfg = {
                            url: Const.aps+'/api/aps/Device/tableTree',
                            data: {
                ...
                $('#btn-del').click(function () {
                    ...
                    if (selResult.success) {
                        ...
                        Mom.layConfirm('确定要删除该指标吗?<br>如果有下级指标将也被删除.',function(layIdx,layero){
                            Api.ajaxForm(Const.aps + '/api/stocktake/StocktakeTemplate/delTempDetail/' + selResult.id, {}, function (result) {
                ...
                function addSonCallback(chkResult, layIdx, layero){
                    if(chkResult.success){
                        ...
                        Api.ajaxForm(Const.aps+"/api/stocktake/StocktakeTemplate/addTempDetail", data, function (result) {
                            if (result.success == true) {
                                Mom.layMsg('保存成功');
                                ztreeLoad();
                                top.layer.close(layIdx);
        ...
        planTempPreserveCVInit: function(){
            ...
            function loadData(){
                if (tempId) {
                    var url, data;
                    if(oper == 'edit'){
                        url = Const.aps + "/api/stocktake/StocktakeTemplate/queryDevice";
                        data = {
                            pId: parentId,
                            stocktakeTemplate: { id: tempId }
                        };
                    }else{
                        url = Const.aps + "/api/aps/Device/queryRootlist";    //选择根指标
                        data = {
            ...
            window.getFormData = function(){
                if(oper == 'edit'){
                    ...
                    return {
                        url: Const.aps +"/api/stocktake/StocktakeTemplate/saveTempDetail",
                        data: {
                            "id": idArr.join(',')
                        }
                    }
                ...
                return {
                    url: Const.aps+"/api/stocktake/StocktakeTemplate/addTempDetail",
                    data: {
                        "templateId": tempId,
                        "detailListStr": JSON.stringify(codeArr)
                    }
