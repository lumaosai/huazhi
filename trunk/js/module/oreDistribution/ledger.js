require([cdnDomain+'/js/zlib/app.js'], function (App) {
    var PageModule = {
        ...
        init: function () {
            ...
            $('#caseId').val(caseName);
            Api.ajaxJson(Const.aps + '/api/ob/LedgerAccount/getOreType?caseId=' + caseId, {}, function (result) {
                if (result.success) {
            ...
            window.pageLoad = function (oreTypeList,isSave) {
                ...
                //渲染表格以及渲染分页
                Api.ajaxJson(Const.aps + "/api/ob/LedgerAccount/list", JSON.stringify(data), function (result) {
        ...
        tablebtn: function () {
            ...
            $("#save-btn").unbind('click').on("click", function () {
                ...
                if (editempty == true) {
                    ...
                    Api.ajaxForm(Const.aps + '/api/ob/LedgerAccount/save', data, function (result) {
                        if (result.success) {
                            Mom.layMsg('保存成功');
                            Api.ajaxJson(Const.aps + '/api/ob/LedgerAccount/getOreType?caseId=' + caseId, {}, function (result) {
                                if (result.success) {
                ...
                if (bol) {
                    top.layer.confirm('请您确认是否要删除勾选数据', {icon: 3, title: '系统提示'}, function (index) {
                        ...
                        if (str.length > 0) { //新增的元素+已存在的数据 或全是已存在的数据
                            ...
                            var url = Const.aps + '/api/ob/LedgerAccount/delete';
            ...
            $('#export-btn').unbind('click').on('click', function () {
                var url = Const.aps + '/ob/import/LedgerAccount/export?caseId=' + caseId + '&oreType=' + $('#oreType option:selected').val() + '&startDate=' + $('#startDate').val() + '&endDate=' + $('#endDate').val() + '&auth=' + Mom.getCookie('authorization') + '&token=' + Mom.getCookie('token_type');
        ...
        opendataCol: function () {
            ...
            p_.layer.open({
                ...
                yes: function (index, layero) { //或者使用btn1
                    ...
                    Api.ajaxForm(Const.aps + '/api/ob/LedgerAccount/getOreType', sdata, function (result) {
        ...
        dataColinit: function () {
            ...
            require(['Page'], function () {
                window.pageLoad = function () {
                    ...
                    var page2 = new Page();
                    page2.pageShowNum = 0;
                    page2.init(Const.aps + "/api/ob/LedgerAccount/collectionData", data, true, function (rows) {
