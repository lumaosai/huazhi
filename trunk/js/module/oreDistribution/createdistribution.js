require([cdnDomain+'/js/zlib/app.js',''], function(App) {
    var PageModule = {
          ...
          PageModule.selectKind();
            ...
            require(['Page'], function () {
                window.pageLoad = function () {
                    function date(date){
                        var formatTimeS = new Date(date).getTime();
                        var TimeS = formatTimeS + 1000*60*60*24*4;
                        ...
                        var defoutDate = dateArr.join('-');
                        $('#endDate1').val(defoutDate);
                    }
                    $('#startDate1').click(function(){
                        WdatePicker({onpicked:function(){date($('#startDate1').val());},maxDate:'#F{$dp.$D(\'endDate1\')}'});
                    })
                    //查看
                    $("#Sequestration").click(function () {
                        if($("#startDate").val() == ""){
                            ...
                            Bus.openDialog("矿石进厂质检信息","oreDistribution/createdistrbutionInner.html?arrivalDate="+$("#startDate").val()+"&arrivalDate2="+$("#endDate").val(),"926px","443px")
                        }
                    });
                    //保存
                    ...
                    $("#export-btn").click(function () {
                        ...
                        var url = Const.aps + '/ob/import/port?id=' + miniCaseId;
                    ...
                    // 计算
                    PageModule.colNumCount();  //默认加载质检信息列表
                    $("#calculation-btn").click(function () {
                        if($("#startDate").val()==""){
                            Mom.layMsg("请选择矿石到货时间！")
                        }else{
                            PageModule.colNumCount();
                        }
                    });
                    //配矿
        ...
        colNumCount:function(){
            ...
            Api.ajaxJson(Const.aps+"/api/ob/QualityInspections/caseCalculate",JSON.stringify(data),function (result) {
                if(result.success){
        ...
        //矿石种类复选框选择
        selectKind:function () {
            var url = Const.admin+"/api/sys/SysDict/type/ORE_TYPE";
        ...
        scheme:function () {
            ...
            if($("#startDate").val() == ""){
                ...
                Api.ajaxJson(Const.aps+"/api/ob/OreBlendingCase/formula",JSON.stringify(data),function (result) {
                    if(result.success){
        ...
        addDataFn:function(){
            ...
            if(num != parseInt($(".heapNum").text())){
                ...
                Api.ajaxJson(Const.aps+"/api/ob/OreBlendingCase/updateCase",JSON.stringify(dataObj),function (result) {
                    if(result.success){
                        Mom.layMsg("保存成功");
                        if($(".exceedbg")){
        ...
        Inner:function (oreType,goodsNum) {
            ...
            if(goodsNums == "day"){
                require(['Page'], function () {
                    new Page().init( Const.aps + "/api/ob/QualityInspections/StatisticsPage", data, true, function (dataTable) {
                        PageModule.createTable1(dataTable);
                    });

                })
            }else{
                Api.ajaxJson(Const.aps + "/api/ob/QualityInspections/StatisticsPage",JSON.stringify(data),function (result) {
                   if(result.success){
