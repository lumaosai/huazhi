require([cdnDomain+'/js/zlib/app.js', ''], function (App) {
        var PageModule = {
            ...
            init: function () {
                ...
                require(['Page'], function () {
                    ...
                    window.pageLoad = function () {
                        ...
                        new Page().init(Const.aps + "/api/ob/QualityInspections/page", data, true, function (result) {
                            ...
                            if (bol) {
                                top.layer.confirm('请您确认是否要删除勾选数据', {icon: 3, title: '系统提示'}, function (index) {
                                    if (str.length > 0) { //新增的元素+已存在的数据 或全是已存在的数据
                                        ...
                                        var url = Const.aps + '/api/ob/QualityInspections/delete';
                        ...
                        $("#save-btn").unbind("click").on('click', function () {
                            ...
                            if (editempty == true) {
                                ...
                                Api.ajaxForm(Const.aps + '/api/ob/QualityInspections/save', data, function (result) {
                                    if (result.success) {
                                        Mom.layMsg('保存成功！');
                                        pageLoad();
                                    } else {
            ...
            //拖拽上传初始化
            importInit: function () {
                require([cdnDomain+'/js/plugins/dropzone/dropzone-amd-module.js', cdnDomain+'/js/plugins/dropzone/dropzone.js'], function (oretype) {
                $('#upexcel').hide();
                var url = Const.aps + '/ob/import/QualityInspections/excelPut/';
                $(".dropzone").dropzone({
                    url: url,//上传地址
