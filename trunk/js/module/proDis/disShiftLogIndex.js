require([cdnDomain+'/js/zlib/app.js'], function(App) {
    ...
    var PageModule = {
        listInit: function(){
            ...
            Api.ajaxJson(Const.aps+'/api/ctrl/ShiftMain/form/DD', JSON.stringify(cz), function (result) {
                ...
                $('.apsType-box').on('click', '.giveWork', function () {
                    if(banName==result.shiftMain.groupName&&banId==result.shiftMain.shift.id){
                        top.layer.confirm('是否确定交班',{
                                ...
                                Api.ajaxForm(Const.aps+'/api/ctrl/ShiftMain/over', data, function (result) {
                                    if(result.success){
                                        top.layer.closeAll();
                ...
                $('.apsType-box').on('click', '.joinWork', function () {
                    if(banName==result.shiftMain.groupName&&banId==result.shiftMain.shift.id){
                        top.layer.confirm('是否确定接班',{
                                ...
                                Api.ajaxForm(Const.aps+'/api/ctrl/ShiftMain/recive', data, function (result) {
                                    if(result.success){
                                        top.layer.closeAll();
                                        window.location.reload()
                                    }else{
            ...
            function renderclass() {
                ...
                var data = {};
                Api.ajaxForm(Const.aps+'/api/ctrl/DirectiveIssued/goupList', data, function (data) {
        ...
        RZHelp: function(){
            function ulClick() {
                renderli(Const.admin+'/api/sys/SysDict/type/jjb_helb_symbol','.symbols');
                renderli(Const.admin+'/api/sys/SysDict/type/jjb_helb_unit','.units');
                $('label').each(function () {
