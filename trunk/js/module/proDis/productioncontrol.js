	 	...
	 	init:function(){
			...
			require(['Page'], function () {
				window.pageLoad = function(){
					...
					new Page().init(Const.aps+"/api/oper/ManageControl/page",data,true,function(tableData, result){
	 	...
	 	createTable:function(tableData){
			...
			btneventHandler:function(){
				...
				$(".btn-delete").unbind("click").on("click",function(){
					var id = $(this).parents('tr').find('.id').text();
					Bus.deleteItem("确定要删除",Const.aps+"/api/oper/ManageControl/deleteBatch",{ids:id});
				});
			},
			//查看
			checkInit:function(){
