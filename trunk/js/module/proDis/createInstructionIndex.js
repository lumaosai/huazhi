    ...
    var PageModule = {
        ...
        dictate: function(){
            ...
            require(['ztree_my'],function(ZTree) {
                ...
                function getOption() {
                    ...
                    Api.ajaxJson(Const.aps+'/api/ctrl/BaseDirective/getZTree',dat,function (da) {
                        zTreeInit(da.rows);
                    });
                    reloadZtree();
                }
            ...
            $('#btn-add').click(function () {
                Bus.openEditDialog('添加指令', './proDis/createInsCV.html?level=1&id=""', '612px', '302px');
            });
            ...
            //获取指令分类
            function ajaxkind() {
                Bus.createSelect(Const.admin+'/api/sys/SysDict/type/directiveCassifyType',$('#kind'),'value','label');
            }
        ...
        orders: function(){
            ...
            window.updatasons = function (id,level){
                ...
                Api.ajaxJson(Const.aps+"/api/ctrl/BaseDirective/rightList/",da,function(tableData){
            ...
            function clickButton(level) {
                $('.btn-addson').click(function(){
                    ...
                    Bus.openEditDialog('添加下级指令','./proDis/createInsCV.html?pid='+id+'&proc='+proc,'612px', '302px');
                });
                $('.btn-add').click(function(){
                    var id = $(this).parents("tr").find('.i-Checks').attr('id');
                    Bus.openEditDialog('添加指令','./proDis/createInsCV.html?&id='+id,'612px', '302px');
                });
                $('.btn-change').click(function () {
                    var id = $(this).parents("tr").find('.i-Checks').attr('id');
                    Bus.openEditDialog('修改指令', './proDis/createInsCV.html?level='+level+'&id=' + id, '612px', '302px');
                });
                $('.btn-delete').click(function () {
                    var id = $(this).parents("tr").find('.i-Checks').attr('id');
                    Bus.deleteItem('确定要删除该指令吗', Const.aps+'/api/ctrl/BaseDirective/delete/'+id);
                });
        ...
        check: function(){
            ...
            if(pid){
                ...
                var url = Const.aps+"/api/ctrl/BaseDirective/form/";
            ...
            //获取工序数据
            function loadProcess(){
                var url=Const.admin+'/api/sys/SysAuthProperty/getAttributeValue/GXJQ/syswp';
            ...
            //获取指令分类
            function ajaxkind() {
                Api.ajaxJson(Const.admin+'/api/sys/SysDict/type/directiveCassifyType',{},function (result) {
                    if(result.success){
                        var rows = result.rows;
