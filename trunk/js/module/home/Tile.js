define([
    'artTemplate',
    cdnDomain+'/js/plugins/gridster/jquery.gridster.js',
    'js/module/home/index-chart.js',
    'SmallPage',
    'tabsNav',
], function(artTemplate,gridster,ec){
    // artTemplate 不支持内置对象，通过helper来操作  v4.10.0以上不支持该方法
    artTemplate.helper("Mathround", function (value) {
    ...
    var register_tile = function(){
        ...
        $(".succesesed_ok").click(function (e) {   //点击对号（保存）的时候
            ...
            Api.ajaxForm(Const.admin+'/api/workbench/WorkbenchHomePage/saveUserTile',data,function(result){
                if(result.success){
    ...
    var load_tile = function(ispush){  //页面加载的时候调用    左侧磁贴管理快捷入口的json数据
        Api.ajaxForm(Const.admin+"/api/workbench/WorkbenchHomePage/list",{},function(result){
            if(result.success){
                ...
                if(tiles.length >0){
                    tiles.forEach(function (tile) {
                        if(ispush){
                            var icon = tile.icon;
                            if(icon && (icon.toLowerCase().indexOf('.jpg')>0 || icon.toLowerCase().indexOf('.png')>0)){
                                tile['imgSrc'] = Const.admin+'/img/sys/SysUpload/showTileImg?id='+tile.tileTemplateId+'&type='+tile.type;
                            }
    ...
    **/
    ...
    var addTileWidget = function(oindex, tile, isFirst, closeDel){
        var html;
        var tileTemplateId='';
        if(tile.tileTemplateId){
            tileTemplateId =tile.tileTemplateId
        }else{
            tileTemplateId = tile.id;
            tile.tileTemplateId = tile.id;
        }
        if($('#tile-' + tile.tileTemplateId).length <= 0){
            if(gridster.length > 0){
                gridster[oindex].add_widget("<li id='tile-" + tileTemplateId + "'  data-ott='" + tileTemplateId + "' class='gs-w'></li>",
        ...
        var styleArr = tile.sizeStyle.split(";");
        for(var i=0;i<styleArr.length-1;i++){
            var arrSplit = styleArr[i].split(":");
            tile[arrSplit[0]] = arrSplit[1].substr(0,arrSplit[1].length-2);
        }
        if(tile.type == 3){
            tile.canvasSizeStyle = getSizeStyle(tile.sizex,tile.sizey,"44");
            html = artTemplate("tmpl-chart",tile);
        }else{
            html = artTemplate("tmpl" + tile.type,tile);
        }
        // var htmlOutput = tmpl.render(tile);
        $(document).find("#tile-" + tileTemplateId).html(html);
        if(closeDel == true){
            $(".group-panel-ul").find(".ingsht,.lbdestlist,.shewidt,.eacstrslist,.circlelist").remove();
        }else{
            $(document).find("#tile-" + tile.id).addClass("groupli-all-" + tile.type);
        }
        switch (tile.type+''){  //2数字类 3图表类 4列表类 5 卡片类  6 文本类 7 自定义  8 图片类
            case "2":
                loadNumberData(tile);
                break;
            case "3":
        ...
        $('.ingsht,.circlelist,.shewidt,.eacstrslist,.lbdestlist').each(function () {
            ...
            $(this).on('click', function () {
                ...
                top.layer.confirm('确认删除该磁贴？', {
                        ...
                        cu_move_tiles.splice($.inArray(id,cu_move_tiles), 1);
                ...
                if (otype == "panel") {
                    ...
                    for(var m = 0, n = cu_panel_tiles.length; m< n; m++){
                        if (cu_panel_tiles[m].id == olist.eq(m).attr("data-ott")) {
    ...
    var store_tiles_unmove = function(tileTemplateId) {
        ...
        for (var i = 0,j = cu_panel_tiles.length?cu_panel_tiles.length:0; i < j; i++) {
            ...
            if(templateCode2 == "panel"){
                panel_tiles_code.push("panel_"+cu_panel_tiles[i].id);
            }else{
    ...
    var appsInit = function() {    //页面每次加载的时候都会调用该方法 然后获取到第一个下拉框的数据 将数据动态的渲染到#apps中
        app_array = [];
        Api.ajaxForm(Const.admin+"/api/workbench/WorkbenchHomePage/appAndTypeList",{},function(result){
            if(result.success){
    ...
    var store_tiles_unmove_style = function() {
        ...
        for (var i = 0,j = store_tiles.length?store_tiles.length:0; i < j; i++) {
            ...
            if(templateCode == "panel"){
                ...
                store_tiles_code.push(ttId,templateCode);
            }
        }
    ...
    var load_tiles_store = function() {    //每次下拉框选择完具体的磁贴类型之后都会调用该方法。
        ...
        Api.ajaxForm(Const.admin + "/api/workbench/WorkbenchHomePage/shopList",data,function(result){
            if(result.success){
                var data = result.allTile;
                store_tiles = data;
                for (var i = 0; i < data.length; i++) {
                    var tile = data[i];
                    tile.templateCode = tile_type;  //shortcut
                    ...
                    app_array.forEach(function (ele,index) {
                        if (ele.appCode == tile.appCode) {
                            ...
                            var icon = tile.icon;
                            if(icon && (icon.toLowerCase().indexOf('.jpg')>0 || icon.toLowerCase().indexOf('.png')>0)){
                                tile['imgSrc'] = Const.admin+'/img/sys/SysUpload/showTileImg?id='+tile.id+'&type='+tileType;
                            }
                            //判断磁贴的类型 1:公有  0:私有
                            if(tile.isPublic == 1){
                ...
                if(tile_type == "panel"){
                    ...
                    htmlOutput = artTemplate("tile_store_panel_tmpl",dataObj);
                    $("#store_group_main").html('<div class="mainCon"><ul class="totleCon" id="store_group_panel">'+htmlOutput+'</ul></div>');
                }else{  //进到这里
                    htmlOutput = artTemplate("tile_store_Tmpl",template);
                    $("#store_group_main").html(htmlOutput);  //默认将磁贴商店下拉框中选中的磁贴集合（个人工作台）的数据动态渲染到#store_group_main容器中
                }
                $('.tiles_total_sort').click(function(){
                    var icon = $(this).children('i.fa');
                    if($(icon).hasClass('fa-sort-desc')){
                        $(icon).removeClass('fa-sort-desc').addClass('fa-sort-up');
                        $(this).css({'margin-top':'-8px'});
                    }else{
                        $(icon).addClass('fa-sort-desc').removeClass('fa-sort-up');
                        $(this).css({'margin-top':'-16px'});
                    }
                   $(this).parent('.title').next('ul.totleCon').slideToggle();
                });
    ...
    var store_tiles_event = function() {
        ...
        $(".totleCon>li .midCon").each(function () {
            ...
            $(this).bind('mouseleave', function (e) {
                e.preventDefault();
                if (getTimeNow() - timeStart > 50 && flagB) {
                    var id = this.id.replace('store_tile_', '');
                    var result = store_tiles_unmove(id);  //判断当前拖拽的元素是否已经存在于容器窗口中
                    if (result) {
                    ...
                    for (var i = 0; i < store_tiles.length; i++) {
                        var st =  store_tiles[i].id;
                        if (st == id) {
                            move_tile = store_tiles[i];  //获取到要移动的磁贴的数据
                        }
                        if(st==undefined && "panel_"+store_tiles[i].id == id){
                    ...
                    $doc.on('mousemove', function (e) {
                        ...
                        var htmlOutput;
                        if(tile_type == "panel"){
                            htmlOutput = artTemplate('panel-tmpl',move_tile);
                        }else{
                            htmlOutput = artTemplate("tmpl" + move_tile.type,move_tile);
                        }
                    ...
                    $doc.one('mouseup', function (e) {  //只绑定一次 只有首次触发事件时会执行该事件处理函数。触发之后，jQuery就会移除当前事件绑定。
                        ...
                        if (is_add) {
                            ...
                            cu_move_tiles.push(move_tile.id);
    ...
    function tileErrormes(tile,errorMsg){
        $('#tile-'+tile.id).find('div.data-div').hide();
        $('#tile-'+tile.id).find('div.error-div').show().html(errorMsg);
    ...
    function ajaxToServer1_quite(url, data, tile, callbackFun){//传送的参数是String时
        var errorContainer = $("#tile-"+tile.tileTemplateId);
        errorContainer.find('.loading-img').show();
        url = url.toLowerCase().indexOf("http://") == 0? url : (Const.admin + url);
        ...
        function loadNumberData_request(){
            ...
            ajaxToServer_quite(url,{},tile,function(result){
                if(result.success == true){
                    ...
                    $("#num" + tile.tileTemplateId).empty().html(data).attr("title",data);
                }else{
                    ...
                    new SmallPage().init({
                        ...
                        for(var j=0; j<rows.length; j++){
                            ...
                            if(operArr.length>0) {
                                ...
                                var dataId = item1.id || item1.ID;
                                bodyHtml += "<td><a data-tileid = '"+tile.tileTemplateId+"' data-id='"+dataId+"' href='javascript:;' onclick='"+opArr[1]+"'>" + opArr[0] + "</a></td>";
                            }
    ...
    /* window.cancelAlarm_callback = function(iframeWin, body,layIdx){
         //调用表单内部方法，提交表单
