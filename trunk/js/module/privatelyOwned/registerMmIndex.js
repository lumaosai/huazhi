require([cdnDomain+'/js/zlib/app.js'], function (App) {
    var PageModule = {
        //注册页面初始化
        regInit: function () {
            Api.ajaxJson(Const.admin + '/api/workbench/WorkbenchTileTemplate/tileTypeList', {}, function (result) {
                if (result.success) {
        ...
        renderRContent: function (data, id) {
            ...
            if ($('#nameImagePreview')) {
                PageModule.initWebuploader('/img/sys/SysUpload/upimg?imgPath=img',
                    '#filePicker',
                    '#nameImagePreview',
                    Const.admin + '/img/sys/SysUpload/showImg/' + id
                );
            };
            //选择颜色 点击执行改变颜色方法
        ...
        initWebuploader: function (url,filePicker, ImagePreview,ImgUrl) {
            require([cdnDomain+'/js/plugins/webUpLoader/webuploader.js'], function (webuploader) {
                ...
                var uploader = webuploader.create({
                    ...
                    auto: true,
                    // // swf文件路径
                    swf: Const.admin + '/plugins/webUploader/Uploader.swf',
        ...
        doSubmit1: function () {
            ...
            Api.ajaxJson(Const.admin+url,JSON.stringify(formdata),function(result){
