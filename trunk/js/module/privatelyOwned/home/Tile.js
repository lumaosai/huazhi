define([
    ...
    'SmallPage',
    'tabsNav',
    ], function(jsrender,artTemplate,gridster,ec){
    ...
    **/
    ...
    var addTileWidget = function(oindex, tile, isFirst, closeDel){
        ...
        if (type == "3") {
            //tmpl = $.templates("#tmpl-chart");
             htmlOutput = artTemplate("#tmpl-chart",tile);
        }else{
            //tmpl = $.templates("#tmpl" + type);
             htmlOutput = artTemplate("#tmpl" + type,tile);
        }
        console.log(tile,999);//对象
        //var htmlOutput = tmpl.render(tile);
        $(document).find("#tile-" + tileTemplateId).html(htmlOutput);
        if(closeDel == true){
        ...
        switch (type){  //2数字类 3图表类 4列表类 5 卡片类  6 文本类 7 自定义  8 图片类
            case "2":
                loadNumberData(tile);
                break;
            case "3":
    ...
    var load_tiles_store = function() {    //每次下拉框选择完具体的磁贴类型之后都会调用该方法。
        ...
        Api.ajaxForm(Const.admin + "/api/workbench/WorkbenchHomePage/shopList",data,function(result){
            if(result.success){
                ...
                console.log(template,888,data); // data = [];只能是对象
                var tmpl;
                var htmlOutput;
                if(tile_type == "panel"){
