require([cdnDomain+'/js/zlib/app.js'], function(App) {
    var PageModule = {
        listInit: function(){
            Api.ajaxForm(Const.admin+'/api/workbench/WorkbenchTileTemplate/typeList',{},rendertitle);
        ...
        mMInit: function(){
            ...
            if(null==typeId || ''==typeId){
                Api.ajaxJson(Const.admin+'/api/workbench/WorkbenchTileTemplate/tileList/',{'typeId':''},function (result) {
                    btnclick(result);
                    renderul(result);
                });
            }else {
                Api.ajaxForm(Const.admin+'/api/workbench/WorkbenchTileTemplate/tileList',{typeId:typeId},function (result) {
                    btnclick(result);
                    renderul(result);
                });
            };
            function btnclick() {
                ...
                $('.notapproval').on('click','.checkView',function () {
                    ...
                    Bus.openDialogCfg('', '/privatelyOwned/mMCheckView.html?typeId='+typeid+'&type='+typecode, '548px', '294px', winOptons);
                });
                /*修改*/
                $('.notapproval').on('click','div.btn-change',function () {
                    ...
                    Bus.openEditDialog('磁贴修改','./privatelyOwned/mMUpdate.html?typeId='+typeid+'&type='+typecode,'725px','654px');
                });
                /*删除*/
                $('.notapproval').on('click','div.btn-delete',function () {
                    ...
                    Bus.deleteItem('是否确定删除？',Const.admin+'/api/workbench/WorkbenchTileTemplate/delete',data,function(result, layerIndex, data){
        ...
        viewDetails: function(){
            ...
            Api.ajaxForm(Const.admin+'/api/workbench/WorkbenchTileTemplate/view', data, function (result) {
                if(result.success){
                    ...
                    if(type==8){
                        ...
                        var imgName='<img src="'+Const.admin+'/img/sys/SysUpload/showTileImg?id='+id+'&type='+type+'"  alt="">';
