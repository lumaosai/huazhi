    ...
    var PageModule = {
        ...
        monthAdjustInit: function () {
            require(['easyui_my'], function (easyui) {
                ...
                Api.ajaxJson(Const.aps + "/api/aps/ApsMonthPc/queryMonthPc", JSON.stringify(data), function (result) {
                    ...
                    $("#btn-count").unbind('click').click(function () { //点击计算按钮
                        ...
                        Api.ajaxJson(Const.aps + "/api/aps/ApsMonthPc/dtpcJs", JSON.stringify(changeVlaArr), function (result) {
                            if (result.success) {
                ...
                function addmsg(data) {
                    Api.ajaxForm(Const.aps + "/api/aps/ApsMonthAdjustTech/saveTechInfo", data, function (result) {
                    })
                }
            });
        },
        dropImportInit: function () {
            require([cdnDomain+'/js/plugins/dropzone/dropzone-amd-module.js', cdnDomain+'/js/plugins/dropzone/dropzone.js'], function () {
                Mom.include('myCss_dropzone', '', [cdnDomain+'/js/plugins/dropzone/dropzone.min.css']);
                $("#download").unbind("click").on("click", function () {
                    location.href = Const.aps + "/aps/PlanExport/zdgzjhExport"
                })
                var url = Const.aps + '/aps/PlanExport/import';
                $(".dropzone").dropzone({
                    url: url,//上传地址
                    paramName: "file",//传参名称
                    maxFilesize: 5.0, // MB
