require([cdnDomain+'/js/zlib/app.js'], function (App) {
    ...
    var PageModule = {
        ...
        measuringNodeInit: function () {
            Api.ajaxJson(Const.mtrl+'/api/fm/NodeMeasure/getNodeAreaList',{},function (result) {
            ...
            require(['Page'], function () {
                ...
                window.pageLoad = function () {
                    ...
                    page.init(Const.mtrl+"/api/fm/NodeMeasure/page", data, true, function (tableData,result) {
        ...
        btngather: function (page) {
            ...
            $('#btn-add').unbind('click').click(function () {
                Bus.openEditDialog('新增计量节点数据', 'material/factoryModels/measuringNodeForm.html', '755px', '380px');
            });
            //头部修改
            $('#btn-edit').unbind('click').click(function () {
                Bus.editCheckedTable('修改计量节点数据', 'material/factoryModels/measuringNodeForm.html', '755px', '380px','#treeTable');
            });
            //计算
            ...
            $('.btn-edit').unbind('click').click(function () {
                var id = $(this).parents("tr").find('.i-checks').attr('id');
                Bus.openEditDialog('修改计量节点数据', 'material/factoryModels/measuringNodeForm.html?id=' + id, '755px', '380px');
            });
            //删除
            $('.btn-delete').unbind('click').click(function () {
                var id = $(this).parents("tr").find('.i-checks').attr('id');
                Bus.deleteItem('确定要删除该计量节点吗', Const.mtrl + '/api/fm/NodeMeasure/del', {ids:id});
            });
            //点击重置按钮
        ...
        measuringNodeCInit: function () {
            ...
            Api.ajaxJson(Const.mtrl+"/api/fm/NodeMeasure/form/"+id,{},function (result){
        ...
        btnComputed: function () {
            ...
            $(thisbox).find('.btn-save').unbind('click').on('click', function () {
                ...
                data['formula']=str;
                Api.ajaxJson(Const.mtrl+'/api/fm/NodeMeasure/updateFormula',JSON.stringify(data),function (result) {
                    if(result.success){
                        Mom.layMsg('保存成功')
                    }else{
