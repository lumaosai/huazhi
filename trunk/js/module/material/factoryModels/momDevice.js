    ...
    var PageModule = {
        momDevice: function(){
            ...
            require(['Page'], function () {
                var page = new Page();
                Api.ajaxJson(Const.mtrl + "/api/fm/AreaUnit/areaUnitList",{},function(result){
                    if(result.success){
                        Bus.appendOptionsValue('#nodeAreaId',result.rows,'id','areaName');
                    }
                });
                Api.ajaxJson(Const.admin + "/api/sys/SysDict/type/UNIT_TYPE",{},function(result){
                    if(result.success){
                ...
                window.pageLoad = function () {
                    ...
                    //修改默认每页显示条数
                    page.init(Const.mtrl + "/api/fm/Unit/page", data, true, function (tableData) {
                        ...
                        $('.btn-delete').click(function () {
                            var id = $(this).parents("tr").find('.i-checks').attr('id');
                            Bus.deleteItem('确定要删除该装置数据吗', Const.mtrl + "/api/fm/Unit/delete", {ids:id});
                        });
                        //编辑
        ...
        momDeviceView: function() {
            ...
            if(id){
                    Api.ajaxJson(Const.mtrl + "/api/fm/Unit/form/"+id,{},function(result){
                        if(result.success){
                ...
                Api.ajaxJson(Const.mtrl + "/api/fm/Unit/form/0",{},function(result){
                    if(result.success){
                        Bus.appendOptionsValue('#unitType',result.unitTypeList,'value','label');
                        Bus.appendOptionsValue('#nodeAreaId',result.areaUnitList,'id','areaName');
                    }
