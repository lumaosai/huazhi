require([cdnDomain+'/js/zlib/app.js'], function (App) {
    ...
    var PageModule = {
        ...
        lateralInit: function () {
            Bus.createSelect(Const.admin+'/api/sys/SysDict/type/SLINE_INOUT_TYPE','#slineInoutType');
            ...
            require(['Page'], function () {
                ...
                window.pageLoad = function () {
                    ...
                    page.init(Const.mtrl + "/api/fm/NodeSideline/page", data, true, function (tableData, result) {
            ...
            Bus.createSelect(Const.mtrl + "/api/fm/Unit/unitList", "#unitId", 'id', 'unitName');
        },
        //创建表格
        ...
        btngather: function (page) {
            ...
            $('#btn-add').unbind('click').click(function () {
                Bus.openEditDialog('新增料线数据', 'material/factoryModels/lateraILForm.html?id=0', '755px', '520px');
            });
            //头部修改
            $('#btn-edit').unbind('click').click(function () {
                Bus.editCheckedTable('修改料线数据', 'material/factoryModels/lateraILForm.html', '755px', '520px', '#treeTable');
            });
            //计算
            ...
            $('.btn-edit').unbind('click').click(function () {
                var id = $(this).parents("tr").find('.i-checks').attr('id');
                Bus.openEditDialog('修改料线数据', 'material/factoryModels/lateraILForm.html?id=' + id, '755px', '520px');
            });
            //删除
            $('.btn-delete').unbind('click').click(function () {
                var id = $(this).parents("tr").find('.i-checks').attr('id');
                Bus.deleteItem('确定要删除该料线吗', Const.mtrl + '/api/fm/NodeSideline/delete', {ids:id});
            });
            //点击重置按钮
        ...
        lateralLCInit: function () {
            ...
            Api.ajaxJson(Const.mtrl+'/api/fm/NodeSideline/viewForm/'+id,{},function (result){
            ...
            require(['/js/module/material/factoryModels/computedCommons.js'], function (computed) {
                ...
                computed.com_selectRead(Const.mtrl+'/api/fm/Formula/instrumentAndFunc');
                //8个切换按钮重新渲染数据// 参数1：头部切换选择了的active的大标签// 参数2：头部切换普通大标签// 参数3：头部切换小标签li
                computed.com_renderChange('.contentBox.active', '.contentBox', '.contentChange>li',Const.mtrl+'/api/fm/Formula/instrumentAndFunc', PageModule.btnComputed);
            })
        },
        //计算公式按钮区
        btnComputed: function () {
            ...
            $(thisbox).find('.btn-save').unbind('click').on('click', function () {
                ...
                Api.ajaxJson(Const.mtrl+'/api/fm/NodeSideline/updateForm',JSON.stringify(data),function (result) {
                    if(result.success){
                        Mom.layMsg('保存成功')
        ...
        lateralLineFormInit: function () {
            ...
            $('#unit').val();
            Api.ajaxJson(Const.mtrl + '/api/fm/NodeSideline/form/' + id, {}, function (result) {
                console.log(result);
                if (result.success) {
