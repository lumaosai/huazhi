    ...
    var PageModule = {
        mutualDonorPoint: function(){
            ...
            require(['Page'], function () {
                var page = new Page();
                Api.ajaxForm(Const.mtrl + "/api/fm/Fctr/fctrSelect",{},function(result){
                    if(result.success){
                ...
                window.pageLoad = function () {
                    ...
                    //修改默认每页显示条数
                    page.init(Const.mtrl + "/api/fm/NodeInnSmt/page", data, true, function (tableData) {
                        ...
                        $('.btn-delete').click(function () {
                            var id = $(this).parents("tr").find('.i-checks').attr('id');
                            Bus.deleteItem('确定要删除该互供点吗', Const.mtrl + '/api/fm/NodeInnSmt/delInnSmt', {ids:id});
                        });
                    });
                };
        ...
        mutualDonorPointView: function() {
            ...
            if(id){
                    Api.ajaxJson(Const.mtrl + "/api/fm/NodeInnSmt/form/" +id,{},function(result){
                        if(result.success){
                ...
                Api.ajaxJson(Const.mtrl + "/api/fm/NodeInnSmt/form/0",{},function(result){
                    if(result.success){
                        var list = result.mtrlFctrSelect;
                        Bus.appendOptionsValue('#srcFctrId',result.mtrlFctrSelect,'id','fctrName');
                        Bus.appendOptionsValue('#destFctrId',result.mtrlFctrSelect,'id','fctrName');
        ...
        btnComputed: function () {
            ...
            $(thisbox).find('.btn-save').unbind('click').on('click', function () {
                ...
                //};
                //data['formula']=str;
                Api.ajaxJson(Const.mtrl+'/api/fm/NodeInnSmt/updateFormula',JSON.stringify(data),function (result) {
                    if(result.success){
                        Mom.layMsg('保存成功')
                    }else{
