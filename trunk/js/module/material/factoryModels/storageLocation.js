    ...
    var PageModel = {
        init:function () {
            require(["Page"],function () {
                ...
                window.pageLoad = function () {
                    ...
                    page.init(Const.mtrl+"/api/fm/NodeLocation/page",data,true,function (result) {
                        PageModel.createTable(result);
                        //编辑按钮
                        $(".tableEdit").unbind("click").on("click",function () {
                            var id = $(this).parents("tr").find('.i-checks').attr('id');
                            Bus.openEditDialog('修改库位数据','../material/factoryModels/storageLocationInner.html?id='+id,'700px','400px');
                        });
                        //删除
                        $(".btn-delete").unbind("click").on("click",function () {
                            var id = $(this).parents("tr").find('.i-checks').attr('id');
                            Bus.deleteItem('确定要删除该库位', Const.mtrl + '/api/fm/NodeLocation/delete',{ids:id});
                        });
                        //新增
                        $("#btn-add").unbind("click").on("click",function () {
                            Bus.openEditDialog('新增库位数据','../material/factoryModels/storageLocationInner.html','700px', '400px')
                        });
                        //编辑
                        $("#btn-edit").unbind("click").on("click",function () {
                            Bus.editCheckedTable('修改库位数据','../material/factoryModels/storageLocationInner.html','700px','400px','#treeTable')
                        });
                        //点击重置按钮
        ...
        checkInit:function () {
            var id = Mom.getUrlParam("id");
             Api.ajaxForm(Const.mtrl+"/api/fm/NodeLocation/form/"+id,{},function (result) {
                if(result.success){
        ...
        loadWarehouse:function () {
            Api.ajaxForm(Const.mtrl + "/api/fm/AreaStock/stockList",{},function(result){
