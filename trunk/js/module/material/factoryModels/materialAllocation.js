   ...
   var PageModule = {
       nodataHtml : "<p class='nodata mgt-10'>暂无数据</p>",
        init:function () {
            ...
            PageModule.loadData(Const.mtrl+"/api/fm/MtrlStock/form/"+0,{},itemArr);
            //给按钮加载事件
            ...
            $("#btn-search1").click(function () {
                ...
                Api.ajaxJson(Const.mtrl+"/api/fm/AreaStock/list",JSON.stringify(data),function (result) {
            ...
            $("#btn-search2").click(function () {
                ...
                Api.ajaxJson(Const.mtrl+"/api/fm/MtrlFctr/findListByFctr",JSON.stringify(data),function (result) {
                var data = [];
                ...
                Api.ajaxJson(Const.mtrl+"/api/fm/MtrlStock/findList",JSON.stringify(dataItem),function (result) {
                    if(result.success){
       ...
       clickLoad:function (ids) {
        ...
        Api.ajaxJson(Const.mtrl+"/api/fm/MtrlStock/findList",JSON.stringify(data),function (result) {
              if(result.success){
                     var data = result.rows;
                     PageModule.createAllocated(data,"list3");
                     PageModule.clickHandler(data);   //操作按钮通过操作数据进行渲染
            ...
            $("#btn-save").unbind("click").on("click",function () {
                   if($("#list1 .active").length<=0 || dataList == undefined){
                       ...
                       if(dataList.length<=0){                                 //如果为空的话
                        ...
                        Api.ajaxForm(Const.mtrl+"/api/fm/MtrlStock/save",data,function (result) {
                            if(result.success){
                                Mom.layMsg("保存成功");
                            }else{
