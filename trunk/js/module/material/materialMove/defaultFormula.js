    ...
    var PageModule = {
        init: function(){
           //加载右侧表http://localhost/json/jqgrid/tableright.json
            Api.ajaxJson(Const.mtrl +"/api/mv/FormulaDef/formType",{}, function (res) {
                if(res.success){
            ...
            //加载左侧表
                window.pageLoad = function () {
                    ...
                    Api.ajaxJson(Const.mtrl +"/api/mv/FormulaDef/form", JSON.stringify(data), function (res) {
                        if(res.success){
                ...
                if (bol) {
                    top.layer.confirm('请您确认是否要删除勾选数据', {icon: 3, title: '系统提示'}, function (index) {
                        if (str.length > 0) { //新增的元素+已存在的数据 或全是已存在的数据
                            ...
                            Api.ajaxForm(Const.mtrl +"/api/mv/FormulaDef/delete", data, function (result) {
                                if (result.success) {
                                    Mom.layMsg('删除成功！');
                                        $('#name').val(''); //清空input
                                        pageLoad();
            ...
            $('#save-btn').unbind('click').click(function(){
                ...
                $("#treeTable tbody tr").each(function(index,item){
                    ...
                    Api.ajaxForm(Const.mtrl +"/api/mv/FormulaDef/save",data,function (result) {
                        if(result.success){
                            Mom.layMsg(result.message);
                            $('#name').val(''); //清空input
                            pageLoad();
