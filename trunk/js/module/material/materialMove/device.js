            ...
            var PageModule = {
                ...
                init: function () {
                    ...
                    pageLoad = function (shift, tankDate, unitId) {
                        ...
                        //是否递交
                        Api.ajaxForm(Const.mtrl + '/api/mv/UnitShiftSline/sealFlag', data, function (result) {
                            if (result.success) {
                                if (result.message == '0') {
                                    $('#unSubmit').hide();
                                ...
                                $('#tankDate').attr('submitType',result.message)
                            } else {
                        ...
                        Api.ajaxJson(Const.mtrl + '/api/mv/UnitProcecaseSwitch/unitProcecase', JSON.stringify(dataSelect), function (result) {
                            if (result.success) {
                                $('#schemeName').empty();
                        ...
                        //列表页
                        Api.ajaxForm(Const.mtrl + "/api/mv/UnitProcecaseSwitch/unitProcecaseSwitch", {unitId: data.unitId}, function (result) {
                            if (result.success) {
                                ...
                                $('#tab-btn').unbind('click').on('click', function () {
                                    if ($('#schemeName').val() == null) {
                                        ...
                                        Api.ajaxJson(Const.mtrl + '/api/mv/UnitProcecaseSwitch/switchUnitProcecase', JSON.stringify(data), function (result) {
                                            if (result.success) {
                                                Mom.layAlert('方案切换成功！')
                                                Api.ajaxForm(Const.mtrl + "/api/mv/UnitProcecaseSwitch/unitProcecaseSwitch", {unitId: data.unitId}, function (result) {
                                                    if (result.success) {
                ...
                btnCollection: function () {
                    ...
                    $('#search-btn-New').unbind('click').on('click', function () {
                        ...
                        $('.tab-btns li').eq(0).addClass('active').siblings('.tab-btns li').removeClass('active');

                        Mom.setCookie(Const.unitDate, $("#tankDate").val()); //保存日期到cookie
                        var dateTime = Mom.getCookie(Const.unitDate)==""?Mom.shortDate:Mom.getCookie(Const.unitDate);

                        $("#tankDate").val(dateTime); //设置日期，如果cookie中有就存cookie的日期，没有保存当前日期
                        $('#tankDate').attr({
                            'createDate': dateTime,
                            'shiftDate': $('#shift').val(),
                    ...
                    $('a.btn-delete').unbind('click').on('click', function () {
                        ...
                        /**改地址*/
                        Bus.deleteItem('确定要删除该方案么', Const.mtrl + '/api/mv/UnitProcecaseSwitch/del', {id: id});
                        Mom.setCookie(Const.unitDate,$('#tankDate').attr('createDate'));
                    });
                    //数据提交
                    $('#submit').unbind('click').on('click', function ()    {
                        ...
                        Mom.setCookie(Const.unitDate,$('#tankDate').attr('createDate'));
                        Api.ajaxForm(Const.mtrl + '/api/mv/UnitShiftSline/submit', data, function (result) {
                            if (result.success) {
                                PageModule.init();
                                $('.tabBtn>li').eq(0).addClass('active').siblings('li').removeClass('active');
                                $('.tab-con>.table-item').eq(0).addClass('active').siblings('.table-item').removeClass('active');
                    ...
                    $('#delClass').unbind('click').on('click', function () {
                        ...
                        Mom.setCookie(Const.unitDate,$('#tankDate').attr('createDate'));
                        Api.ajaxForm(Const.mtrl + '/api/mv/TankSeal/deleteShift',data, function (result) {
                            if (result.success) {
                                PageModule.init();
                                $('.tabBtn>li').eq(0).addClass('active').siblings('li').removeClass('active');
                                $('.tab-con>.table-item').eq(0).addClass('active').siblings('.table-item').removeClass('active');
                ...
                timeAndZtree: function (callback) {
                    ...
                    require(['ztree_my'], function (ZTree) {
                        ...
                        var ztree = new ZTree();
                        //调取接口拿到ztree数据
                        Api.ajaxJson(Const.mtrl + '/api/fm/Unit/unitList', {}, function (result) {
                            if (result.success) {
                                //配置ztree参数
                                var ztreeSetting = {
                                    data: {
                    ...
                    $(lis).each(function (index, item) {
                        $(item).unbind("click").on("click", function () {
                            ...
                            if (index == 1 || index == 2 || index == 3) {
                                $('.treeTable-count').addClass('hide');
                                if (index == 1) {
                                    //班计量
                                    $('#treeTable').parent('div').empty();
                                    PageModule.pageChange('jqgrid', '.classMeasure', 'UnitShiftInstrument', PageModule.renderJqgrid)
                                } else if (index == 2) {
                                    $('#UnitShiftInstrument').parent('div').empty();
                                    //投入产出
                ...
                renderDevice: function (tableData) {
                    $('#Unit').dataTable({
                        ...
                        "aoColumns": [
                            ...
                            {"data": "switchDate", 'sClass': "center", "width": "34%"},
                            {
                                "data": "id",
                                "orderable": false,
                                "defaultContent": "",
                ...
                renderJqgrid: function (isgather, data) {
                    require(['jqGrid_my'], function (jqGridAll) {
                        window.pageLoadclass = function () {
                            if (isgather == true) {
                                var url = Const.pi + "/api/PiApi/tagNearLocal";
                                var dataList = data;
                                Api.ajaxForm(url, dataList, function (result) {
                                    var url = Const.mtrl + "/api/mv/UnitShiftInstrument/queryUnitShiftInstrumentList";
                                    //————————改为默认加载地址————————//
                                    var dataList = {
                                        unitId: $('#tankDate').attr('unitId'),
                                        date: $('#tankDate').attr('createDate'),
                                    ...
                                    if (res.success) {
                                        ...
                                        var optionsPot = {
                                            ...
                                            gridComplete:function(){
                                                // $("#UnitShiftInstrument").closest(".ui-jqgrid-bdiv").css({ 'overflow': 'hidden' });
                                        ...
                                        var optionsMMove = {
                                            colModel: [
                                                ...
                                                {name: "saveFlag", label: "F", align: "center", title: false, width: "30"},
                                                {
                                                    name: "materialName",
                                                    label: "料线名称",
                                                    align: "center",
                                                ...
                                                },
                                                {
                                                    ...
                                                    formatter: function (cellvalue, options, rowObject) {
                                                        var optionArr=[];
                                                        RankTypeSel.forEach(function (item,i) {
                                                            var selted=rowObject.rankType==item.value?'selected':'';
                                                            optionArr.push('<option value="'+item.value+'" '+selted+'>'+item.label+'</option>')
                                                        });
                                                        if($('#tankDate').attr('submitType')=='0'){
                                                            var html='<select class="form-control editSelect select2">'+optionArr.join('')+'</select>';
                                                        }else if($('#tankDate').attr('submitType')=='1'){
                                                            var html='<select class="form-control editSelect select2" disabled="disabled" >'+optionArr.join('')+'</select>';
                                                        }
                                        ...
                                        var config = {
                                            url: Const.mtrl + "/api/mv/UnitShiftInstrument/getSlines",
                                            dataParams: {
                                                unitId: $('#tankDate').attr('unitId'),
                                                date: $('#tankDate').attr('createDate'),
                                                shift: $('#tankDate').attr('shiftDate')
                                        ...
                                        jqGridAll.jG_jqGridTableLevel('#UnitShiftInstrument', optionsPot, optionsMMove, config, MMovesubTable, function (subtabIds, datajson, resdata) {
                                            $('.editSelect').on('change', function () {
                                                ...
                                                Api.ajaxJson(Const.mtrl + '/api/mv/UnitShiftInstrument/saveSlines', JSON.stringify(data), function (result) {
                                                    if (result.success) {
                                                        Mom.layMsg('修改成功');
                                                    } else {
                                                        Mom.layAlert(result.message);
                                        ...
                                        $('.leftBtn').each(function (i, o) {
                                            $('#UnitShiftInstrument').off('click', 'a.leftBtn').on('click', 'a.leftBtn', function (event) {
                                                ...
                                                Api.ajaxForm(Const.mtrl + '/api/mv/UnitShiftInstrument/frontVal', data, function (result) {
                                                    if (result.success) {
                                        ...
                                        $('#class-btn-save').unbind('click').on('click', function () {
                                            ...
                                            Mom.setCookie(Const.unitDate,$('#tankDate').attr('createDate'));
                                            Api.ajaxForm(Const.mtrl + '/api/mv/UnitShiftInstrument/save', data, function (result) {
                                                if (result.success) {
                                                    Mom.layMsg('保存成功')
                                                } else {
                                                    Mom.layAlert(result.message);
                                        ...
                                        $('#classMeasure').find('.treeTable-count').removeClass('hide');
                                    } else {
                ...
                inputOutput: function () {
                    require(['jqGrid_my'], function (jqGridAll) {
                        ...
                        window.iOpageLoad = function (result) {
                            ...
                            Api.ajaxForm(Const.mtrl + "/api/mv/UnitShiftSline/list", dataList, function (res) {
                                resultList = res;
                                if (result) {
                                    res = result;
                                }
                                if (res.success) {
                                    $('#inputOutput').find('.treeTable-count').removeClass('hide');
                                    ...
                                    var config = {
                                        url: Const.mtrl + '/api/mv/UnitShiftSline/view',
                                        otherId: 'nodeId',
                                        dataParams: {
                                            createDate: $('#tankDate').attr('createDate'),//日期
                                            shiftDate: $('#tankDate').attr('shiftDate'),//班次
                                    ...
                                    var subtable = [];
                                    jqGridAll.jG_jqGridTableLevel('#treeTable', optionsPot, optionsMMove, config, subtable,false,1);
                                } else {
                        ...
                        $('#btn-comAgain').unbind('click').click(function () {
                            ...
                            $('#inputOutput').find('.treeTable-count').addClass('hide');
                            var dataList = {
                                createDate: $('#tankDate').attr('createDate'),//日期
                                shiftDate: $('#tankDate').attr('shiftDate'),//班次
                                unitId: $('#tankDate').attr('unitId')
                            ...
                            Api.ajaxForm(Const.mtrl + "/api/mv/UnitShiftSline/recalculate", dataList, function (result) {
                                iOpageLoad(result);
                        ...
                        $('#btn-save').unbind('click').click(function () {
                            ...
                            Mom.setCookie(Const.unitDate,$('#tankDate').attr('createDate'));
                            Api.ajaxForm(Const.mtrl + "/api/mv/UnitShiftSline/save", da, function (res) {
                                if (res.success) {
                                    Mom.layMsg(res.message);
                                    $('.inputOutput').empty().html(html);
                                    $('#inputOutput').find('.treeTable-count').addClass('hide');
                ...
                renderMaterialM: function () {
                    ...
                    Api.ajaxForm(Const.mtrl + "/api/mv/UnitMtrlMove/list/" + data.id, data, function (result) {
                        if (result.success) {
                            //渲染成功多少条
                            ...
                            //按钮集合
                            PageModule.btnCollection();
                            $('#materialMove').find('.treeTable-count').removeClass('hide');
                            //删除
                                ...
                                if (bol) {
                                    top.layer.confirm('请您确认是否要删除勾选数据', {icon: 3, title: '系统提示'}, function (index) {
                                        if (str.length > 0) { //新增的元素+已存在的数据 或全是已存在的数据
                                            ...
                                            var url = Const.mtrl + '/api/mv/UnitMtrlMove/delete';
                ...
                sceneGraph: function () {
                    ...
                    var shiftDate =  $('#tankDate').attr('shiftDate');
                    var unitId =     $('#tankDate').attr('unitId');
                    var html = '<embed src="'+Const.mtrl+'/mv/Topo/unit/'+unitId+'/'+createDate+'/'+shiftDate+'" width="100%" height="450" type="image/svg+xml" />';
                    $('#sceneGraph').empty();
                ...
                decMMAForm: function () {
                    ...
                    if (type == 'add') {
                        ...
                        function ajaxtype(type, callback) {
                            ...
                            Api.ajaxJson(Const.mtrl + '/api/mv/UnitMtrlMove/form/' + data.id, data, function (result) {
                                if (result.success) {
                                    if (callback) {
                                        callback(result)
                                    }
                        ...
                        function changefun(isCheck) {
                            ...
                            Api.ajaxJson(Const.mtrl + '/api/mv/UnitMtrlMove/getListByType', JSON.stringify(subdata), function (result) {
                                if (result.success) {
...
//本方节点变化的时候
                    ...
                    //关闭 走的是自己的方法
                        ...
                        Api.ajaxJson(Const.mtrl + '/api/mv/UnitMtrlMove/view/' + data.id, data, function (result) {
                    ...
                    window.Submit = function (substatu, moveType, layerIdx, callback) {
                        ...
                        if (moveType == 'add') {
                            var url = Const.mtrl + '/api/mv/UnitMtrlMove/save';
                        } else if (moveType == 'close') {
                            var url = Const.mtrl + '/api/mv/UnitMtrlMove/update';
                        }
