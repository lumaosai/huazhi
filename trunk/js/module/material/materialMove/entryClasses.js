        ...
        init: function(){
            require(['jqGrid_my'], function (jqGridAll) {
                ...
                window.pageLoad = function (result) {
                    ...
                    Api.ajaxForm(Const.mtrl +"/api/mv/ShiftVal/shiftStatus", dataList, function (da) {
                    ...
                    Api.ajaxForm(Const.mtrl +"/api/mv/ShiftVal/queryCurrShift", dataList, function (res) {
                        if(res.success){
                            if(result){
                                res = result;
                            }
                            $('.mgt-10').removeClass('hide');
                            ...
                            var colModel2 = [
                                ...
                                {"name": "preTime","label": "前量时间","align": "center","width":"300"},
                                {"name": "aftCont","label": "后量","align": "center"},
                                {"name": "aftTime","label": "后量时间","align": "center","width":"300"}
                            ];
                            var optionsMMove = {   //子表
                                colModel: colModel2,
                                rownumbers: true,
                            ...
                            var config = {
                                url: Const.mtrl+'/api/mv/ShiftVal/queryDetail',
                                otherId:'nodeId',
                                dataParams:{
                                    date:Mom.getCookie(Const.enterDate),//日期
                                    shift:$("#shiftHidden").val(),//班次
                ...
                $('#btn-extract').unbind('click').click(function(){
                    ...
                    Api.ajaxForm(Const.mtrl+"/api/mv/ShiftVal/extract", dataList, function (res) {
                        if(res.success){
                            $('.table-content').empty().html(html);
                            $('.mgt-10').addClass('hide');
                            pageLoad(res);
                ...
                $('#save-btn').unbind('click').click(function () {
                    ...
                    Api.ajaxForm(Const.mtrl+"/api/mv/ShiftVal/save", dataList, function (res){
                        if(res.success){
                ...
                $('#btn-submit').unbind('click').click(function(){
                    ...
                    Api.ajaxForm(Const.mtrl+"/api/mv/ShiftVal/submitShift", dataList, function (res){
                        if(res.success){
                            $('.table-content').empty().html(html);
                            $('.mgt-10').addClass('hide');
                            pageLoad();
                ...
                $('#btn-relieve').unbind('click').click(function(){
                    ...
                    Api.ajaxForm(Const.mtrl+"/api/mv/ShiftVal/relieve", dataList, function (res){
                        if(res.success){
                            $('.table-content').empty().html(html);
                            $('.mgt-10').addClass('hide');
                            pageLoad();
