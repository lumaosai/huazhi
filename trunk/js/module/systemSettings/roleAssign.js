    ...
    var PageModule = {
        ...
        listAllotPro: function(){
            ...
            require(['Page'], function () {
                ...
                window.pageLoad = function () {
                    ...
                    page.init(Const.admin+"/api/sys/SysRole/rolePermPage", data, true, function (tableData, result) {
                ...
                $('#allot-btn').click(function () {
                    allotCancelItem("#treeTable", "dist", "是", Const.admin+"/api/sys/SysRole/ajaxSaveDist");
                });
                //撤销
                $('#cancel-btn').click(function () {
                    allotCancelItem("#treeTable", "cancel", "否", Const.admin+"/api/sys/SysRole/ajaxSaveDist");
                });
                //点击重置按钮
        ...
        roleAssign: function(){
            ...
            function refreshLoad(){
                if(roleId != ''){
                    var url = Const.admin+"/api/sys/SysRole/view/"+roleId;
                    ...
                    //用户功能显示（已分配）用户信息列表
                    var url = Const.admin+"/api/sys/SysRole/assign/" + roleId;
                    Api.ajaxJson(url, {}, function (result) {
                        if (result.success) {
            ...
            function deleteACallback(msg, data){
                Mom.layConfirm(msg, function(layIdx, layero){
                    Api.ajaxForm(Const.admin+'/api/sys/SysRole/removeUser', data, function (result) {
                        if (result.success == true) {
                            refreshLoad();
            ...
            $("#assignButton").click(function(){
                var options = {
                    hasUserOptions:{
                        apiCfg:{
                            url: Const.admin+"/api/sys/SysRole/assign/"+roleId,
                        }
                    }
                }
                Bus.openSelUserWin3('选择用户', options, function(selResult, layIdx, layero){
                    if(selResult.success){
                        ...
                        Api.ajaxForm(Const.admin+'/api/sys/SysRole/saveUserRole', formdata, function(result){
                            if (result.success == true) {
