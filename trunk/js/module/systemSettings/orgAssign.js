require([cdnDomain+'/js/zlib/app.js'], function(App) {
    var PageModule = {
        orgAssign: function(){
            ...
            if(id!=='') {
                $("#orgId").val(id);
                Api.ajaxForm(Const.admin+'/api/sys/SysOrg/view/'+id, {}, function(result){
                    if (result.success) {
            ...
            require(['Page'],function(){
                function refreshLoad(){
                    ...
                    var page = new Page();
                    page.pageShowNum=0;
                    page.init(Const.admin+"/api/sys/SysUser/page",data,true,function (rows, result) {
                        if (result.success) {
                ...
                $("#assignButton").click(function(){
                    ...
                    Bus.openSelUserWin3('选择用户', options, function(selResult, layIdx, layero){
                        if(selResult.success){
                            ...
                            Api.ajaxForm(Const.admin+'/api/sys/SysOrg/addUser', formdata, function(result){
                ...
                function deleteACallback(msg, data){
                    Mom.layConfirm(msg, function(layIdx, layero){
                        Api.ajaxForm(Const.admin+'/api/sys/SysOrg/delUser', data, function (result) {
                            if (result.success == true) {
                                refreshLoad();
                                top.layer.close(layIdx);
                            }
        ...
        orgAllotApp: function(){
            ...
            if(orgId!=='') {
                $("#orgId").val(orgId);
                Api.ajaxForm(Const.admin+'/api/sys/SysOrg/view/'+orgId, {}, function(result){
                    if (result.success) {
                        Validator.renderData(result.SysOrg, $('#orgData'));
                    } else {
                        Mom.layMsg(result.message)
            ...
            function refreshLoad(){
                Api.ajaxJson(Const.admin+"/api/sys/SysOrg/assignApp/"+orgId, {}, function(result){
                    if (result.success) {
            ...
            $("#assignButton").click(function () {
                Bus.openEditDialog('选择应用','systemSettings/selectApp.html?orgId='+orgId,'800px','545px',function(layerIdx,layero){
                    ...
                    if(formData){
                        ...
                        Api.ajaxForm(Const.admin+'/api/sys/SysOrg/addApp',data,function(result) {
                            if (result.success) {
                                refreshLoad();
            ...
            function deleteACallback(msg, data){
                Mom.layConfirm(msg, function(layIdx, layero){
                    var url = Const.admin+'/api/sys/SysOrg/delApp';
        ...
        selectApp: function(){
            ...
            require(['Page'],function(){
                ...
                window.pageLoad= function () {
                    ...
                    page.pageShowNum = 0;
                    page.init(Const.admin+"/api/sys/SysOrg/selectApp",data, true, function(tableData){
                        dataout(tableData);
                    });
                    //表格渲染
