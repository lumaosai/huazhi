    ...
    var PageModule = {
        listInit: function(){
            ...
            require(['easyui_my'],function(easyuiObj){
                ...
                $('#delete-btn').click(function(){
                    ...
                    if(idArr.length>0){
                        Mom.layConfirm('要删除该菜单及所有子菜单项吗？',function(layerIdx, layero){
                            Api.ajaxForm(Const.admin + '/api/sys/SysMenu/ajaxDel', {ids:idArr.join(',')}, function(result){
                ...
                $('#updateSort-btn').unbind().click(function(){
                    ...
                    if(ids.length>0){
                        ...
                        Api.ajaxForm(Const.admin + "/api/sys/SysMenu/ajaxSort",data,function(result){
                            if(result.success){
                                pageLoad();
                            }else{
                                Mom.layAlert(result.message);
                ...
                function pageLoad(node) {
                    Api.ajaxJson(Const.admin+"/api/sys/SysMenu/getTreeGraidJson",{},function(result){
                        if(result.success){
                            $('#tt').treegrid({
                                idField:'id',
                                treeField:'name',
            ...
            $("#sync-btn").click(function(){
                Mom.layConfirm('确定继续吗?',function(){
                    Api.ajaxForm(Const.admin+"/api/sys/SysMenu/ajaxSys",{},function(result){
                        Mom.layAlert(result.message);
                    });
                });
            });
        ...
        menuInner: function(){
            ...
            if( id != "" || pid != "" ){
                ...
                var url = Const.admin+"/api/sys/SysMenu/form";
            ...
            $("#menuButton").click(function(){
                var apiCfg = { url: Const.admin+'/api/sys/SysMenu/openTree', data:{} };
